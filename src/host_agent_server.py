...
from aiohttp.web import AppKey, Application, Request, Response, json_response, run_app
...
APP_KEY_AGENT_CONFIGURATION = AppKey("agent_configuration", AgentAuthConfiguration | None)
APP_KEY_AGENT_APP = AppKey("agent_app", AgentApplication)
APP_KEY_ADAPTER = AppKey("adapter", CloudAdapter)
...
class GenericAgentHost:
    ...
    def _setup_handlers(self):
        ...
        # ISS-017: consent_required was previously blocking the handler from firing;
        # this was resolved by granting OAuth2 consent to the agent app instance.
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None):
        async def entry_point(req: Request) -> Response:
            return await start_agent_process(
                req, req.app[APP_KEY_AGENT_APP], req.app[APP_KEY_ADAPTER]
            )

        async def messages_health(_req: Request) -> Response:
        ...
        async def health(_req: Request) -> Response:
            ...
            if self.agent_instance and hasattr(self.agent_instance, "get_mcp_health_snapshot"):
                try:
                    status["mcp"] = self.agent_instance.get_mcp_health_snapshot()
                except Exception as e:
                    status["mcp"] = {"state": "error", "last_error": str(e)}
        ...
        app[APP_KEY_AGENT_CONFIGURATION] = auth_configuration
        app[APP_KEY_AGENT_APP] = self.agent_app
        app[APP_KEY_ADAPTER] = self.agent_app.adapter

        app.on_startup.append(lambda app: self.initialize_agent())
        app.on_shutdown.append(lambda app: self.cleanup())

        desired_port = int(environ.get("PORT", 8000))
        port = desired_port
